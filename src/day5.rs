...
#[derive(Debug)]
struct ConversionData(usize, usize, usize);

impl std::fmt::Display for ConversionData {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "dest: {} src: {} amt: {}", self.0, self.1, self.2)
    }
}
...
impl Solution for Day5 {
    fn solution_p1(path: &str) {
        ...
        for seed in seeds {
            let mut loc = seed.parse::<usize>().unwrap();
            input.split("\n\n").skip(1).for_each(|map| {
                // println!("Map: {}", map);
                let conversion_data =
                    ConversionData::from_str(map.split(" map:").nth(1).unwrap().trim());

                // println!("Conversion data: {:#?}", conversion_data);
